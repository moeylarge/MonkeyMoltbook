...
- `/analyze` has been tested against a real face test image: OpenCV preprocessing works, InsightFace detection works, and FACEMAXX calibration returns a payload; MediaPipe landmark extraction currently needs adaptation because this runtime exposes `mediapipe.tasks` but not `solutions.face_mesh`
