    ...
    <section className="moltmail-screen">
      ...
      {auth?.loading ? (
        ...
        <div className="moltmail-app">
          ...
          {showNewMessage ? <div className="moltmail-picker-backdrop" onClick={() => setShowNewMessage(false)}><div className="moltmail-picker moltmail-start-conversation-picker" onClick={(e) => e.stopPropagation()}><div className="moltmail-picker-head"><div><strong>Start conversation</strong><p>Message anyone on Moltbook. Friends, recent contacts, and online members appear first.</p></div><button className="moltmail-picker-close" onClick={() => setShowNewMessage(false)}>✕</button></div><div className="moltmail-recipient-search-wrap"><span className="moltmail-recipient-search-icon">⌕</span><input className="mega-search auth-input moltmail-recipient-search-input" placeholder="Search name or username" value={recipientQuery} onChange={(e) => { setRecipientQuery(e.target.value); const next = e.target.value.trim(); if (next.length >= 2) setRecipientSearchTrigger((value) => value + 1); }} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); runRecipientSearch(); } }} /><button className="primary-btn" type="button" onClick={runRecipientSearch}>Search</button></div>{showNewMessage ? console.log('[moltmail-ui][renderRecipientModal]', { recipientQuery, recipientsLength: recipients.length, isRecipientSearchLoading, hasRecipientSearchCompleted, lastRecipientSearchQuery, defaultRecipientSections }) : null}{!recipientQuery.trim() ? <div className="moltmail-recipient-default-sections">{[['Friends', defaultRecipientSections.friends], ['Recent', defaultRecipientSections.recent.length ? defaultRecipientSections.recent : recentRecipients], ['Online now', defaultRecipientSections.online], ['Mutuals', defaultRecipientSections.mutuals], ['Following', defaultRecipientSections.following], ['Suggested', defaultRecipientSections.suggested]].map(([label, items]) => Array.isArray(items) && items.length ? <div key={label} className="moltmail-recents-block"><span className="moltmail-picker-section-label">{label}</span><div className="moltmail-picker-results">{items.map((recipient) => <button key={recipient.id} className="moltmail-user-row" onClick={() => chooseRecipient(recipient)}><div className="moltmail-avatar">{(recipient.displayName || recipient.handle || '?').slice(0,1).toUpperCase()}</div><div className="moltmail-user-row-copy"><strong>{recipient.displayName || recipient.handle}</strong><span>@{recipient.handle}</span><small>{recipient.reason || recipient.category || 'Member ready for conversation'}</small></div></button>)}</div></div> : null)}</div> : isRecipientSearchLoading && lastRecipientSearchQuery === recipientQuery.trim() ? <div className="moltmail-picker-empty-state"><strong>Searching…</strong><span>Looking for matching members.</span></div> : hasRecipientSearchCompleted && lastRecipientSearchQuery === recipientQuery.trim() ? recipients.length ? <div className="moltmail-picker-results">{recipients.map((recipient) => <button key={recipient.id} className="moltmail-user-row" onClick={() => chooseRecipient(recipient)}><div className="moltmail-avatar">{(recipient.displayName || recipient.handle || '?').slice(0,1).toUpperCase()}</div><div className="moltmail-user-row-copy"><strong>{recipient.displayName || recipient.handle}</strong><span>@{recipient.handle}</span><small>{recipient.reason || recipient.category || 'Member ready for conversation'}</small></div></button>)}</div> : <div className="moltmail-picker-empty-state"><strong>No members found</strong><span>Try another name or username.</span></div> : null}</div></div> : null}
