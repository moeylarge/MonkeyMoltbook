            ...
            <div className="member-profile-topbar">
              <div className="member-profile-identity-row">
                ...
                <div className="member-profile-identity-copy">
                  ...
                  {previewProfile?.location_text || previewProfile?.pronouns || joinedLabel || trustSignal || previewProfile?.updated_at ? <div className="member-profile-inline-meta">
                    {trustSignal ? <span>{trustSignal}</span> : null}
                    {joinedLabel ? <span>Joined {joinedLabel}</span> : null}
                    {previewProfile?.updated_at ? <span>Updated recently</span> : null}
...
function MoltMailPage({ auth, onOpenAuth, onTrackClick }) {
  ...
  const [defaultRecipientSections, setDefaultRecipientSections] = useState({ friends: [], recent: [], online: [], mutuals: [], following: [], suggested: [] });
  ...
  useEffect(() => {
    ...
    if (!trimmedQuery) {
      ...
      fetch(`${API}/moltmail/recipients/default`, { credentials: 'include' })
        .then((res) => res.json())
        .then((json) => setDefaultRecipientSections(json?.sections || { friends: [], recent: [], online: [], mutuals: [], following: [], suggested: [] }))
        .catch(() => setDefaultRecipientSections({ friends: [], recent: [], online: [], mutuals: [], following: [], suggested: [] }));
      return;
    }
    let active = true;
  ...
  const chooseRecipient = async (recipient) => {
    try {
      const response = await fetch(`${API}/moltmail/recipient/${recipient.id}/thread`, { credentials: 'include' });
      const payload = await response.json().catch(() => ({}));
    ...
    <section className="moltmail-screen">
      ...
      {auth?.loading ? (
        ...
        <div className="moltmail-app">
          ...
          {showNewMessage ? <div className="moltmail-picker-backdrop" onClick={() => setShowNewMessage(false)}><div className="moltmail-picker moltmail-start-conversation-picker" onClick={(e) => e.stopPropagation()}><div className="moltmail-picker-head"><div><strong>Start conversation</strong><p>Pick someone below or search all members.</p></div><button className="moltmail-picker-close" onClick={() => setShowNewMessage(false)} aria-label="Close start conversation">✕</button></div><div className="moltmail-recipient-search-wrap"><span className="moltmail-recipient-search-icon">⌕</span><input className="mega-search auth-input moltmail-recipient-search-input" placeholder="Search name or username" value={recipientQuery} onChange={(e) => { setRecipientQuery(e.target.value); const next = e.target.value.trim(); if (next.length >= 2) setRecipientSearchTrigger((value) => value + 1); }} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); runRecipientSearch(); } }} /></div>{!recipientQuery.trim() ? <div className="moltmail-recipient-default-sections">{[['Recent', defaultRecipientSections.recent.length ? defaultRecipientSections.recent : recentRecipients], ['Online now', defaultRecipientSections.online], ['Suggested', defaultRecipientSections.suggested]].map(([label, items]) => Array.isArray(items) && items.length ? <div key={label} className="moltmail-recents-block"><span className="moltmail-picker-section-label">{label}</span><div className="moltmail-picker-results">{items.slice(0, 8).map((recipient) => <button key={recipient.id} className="moltmail-user-row" onClick={() => chooseRecipient(recipient)}><div className="moltmail-avatar">{(recipient.displayName || recipient.handle || '?').slice(0,1).toUpperCase()}</div><div className="moltmail-user-row-copy"><strong>{recipient.displayName || recipient.handle}</strong><span>@{recipient.handle}</span><small>{recipient.reason || recipient.category || 'Member ready for conversation'}</small></div><div className="moltmail-user-row-actions"><span className="moltmail-user-row-cta">Message</span></div></button>)}</div></div> : null)}</div> : isRecipientSearchLoading && lastRecipientSearchQuery === recipientQuery.trim() ? <div className="moltmail-picker-empty-state"><strong>Searching…</strong><span>Looking for matching members.</span></div> : hasRecipientSearchCompleted && lastRecipientSearchQuery === recipientQuery.trim() ? recipients.length ? <div className="moltmail-picker-results">{recipients.map((recipient) => <button key={recipient.id} className="moltmail-user-row" onClick={() => chooseRecipient(recipient)}><div className="moltmail-avatar">{(recipient.displayName || recipient.handle || '?').slice(0,1).toUpperCase()}</div><div className="moltmail-user-row-copy"><strong>{recipient.displayName || recipient.handle}</strong><span>@{recipient.handle}</span><small>{recipient.reason || recipient.category || 'Member ready for conversation'}</small></div><div className="moltmail-user-row-actions"><span className="moltmail-user-row-cta">Message</span></div></button>)}</div> : <div className="moltmail-picker-empty-state"><strong>No members found</strong><span>Try another name or username.</span></div> : <div className="moltmail-picker-empty-state"><strong>Type at least 2 characters</strong><span>Search the full Moltbook member graph.</span></div>}</div></div> : null}
