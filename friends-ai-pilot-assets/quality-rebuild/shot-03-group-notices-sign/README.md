# Shot 03 — Group Notices Sign Package
...
Per `friends-ai-pilot-assets/MOTION_PHASE_HANDOFF.md`:
- the old “group notices sign” asset should **not** be treated as a character-motion shot
- it should remain a still insert unless a true six-character noticing-sign frame is generated later

## Best existing reference assets
1. Legacy story asset (audit memory only):
   - `references/03-group-notices-sign-story-shot.png`
2. Legacy upload-ready insert (audit memory only):
...
Pass 2 proved the package can now suppress signage/artifact contamination, but it still fails if the bound source does not pre-lock a single shared notice vector.
...
- one unified off-frame notice-direction eyeline across the ensemble
