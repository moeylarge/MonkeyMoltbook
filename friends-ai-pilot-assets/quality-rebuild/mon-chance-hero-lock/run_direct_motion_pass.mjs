...
const promptPath = path.join(heroDir, 'motion-prompt-v1.md');
const outDir = path.join(heroDir, 'motion-clips/2026-03-25');
const stamp = 'mon-chance-2026-03-25-motion-pass-1-veo2';
...
async function fetchJson(url, options) {
  ...
  if (!res.ok) {
    throw new Error(`HTTP ${res.status} ${res.statusText}: ${JSON.stringify(data).slice(0, 2000)}`);
  }
  return data;
}
...
async function main() {
  ...
  if (!videoUrl) {
    throw new Error(`No video URL in response: ${JSON.stringify(data).slice(0, 3000)}`);
  }
