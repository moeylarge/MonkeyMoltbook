...
const promptPath = path.join(heroDir, 'hero-still-prompt-v4-surgical.md');
const outDir = path.join(heroDir, 'outputs/2026-03-25');
const stamp = '2026-03-25-direct-pass-2';
...
async function main() {
  ...
  if (!data.images && requestId) {
    data = await fetchJson(`https://fal.run/fal-ai/nano-banana-pro/edit/requests/${requestId}`, {
      headers: {
        'Authorization': `Key ${falKey}`,
        'Accept': 'application/json'
      }
    ...
    if (!data.images) {
      data = await fetchJson(`https://fal.run/fal-ai/nano-banana-pro/edit/requests/${requestId}/status`, {
        headers: {
          'Authorization': `Key ${falKey}`,
          'Accept': 'application/json'
        }
